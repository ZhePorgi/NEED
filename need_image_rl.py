#!/usr/bin/env python3
"""NEED image preference training and low-data image policy tuning.

This module trains from image preference pairs: expand them conservatively,
train a small reward surface, or semi-finetune only image-facing NEED parameters
with a DPO-like pairwise loss. It does not generate images; it only trains from
supplied image preference data.
"""
...
def augment_preference_row(row: PreferenceRow, factor: int = 3) -> List[PreferenceRow]:
    """Conservative preference row expansion.

    Do not invent new labels or alter the chosen/rejected relationship.  The
    extra rows vary prompt wrappers and carry augmentation ids so optional
    paired pixel transforms can be deterministic.
    """
...
def train_reward(args: argparse.Namespace) -> None:
    ...
    if not args.preferences:
        raise ValueError("Provide --preferences")
...
class ImagePolicyPreferenceDataset(Dataset):
    def __init__(
        ...
        self.tok = tokenizer if tokenizer is not None else ByteTokenizer()
        self.grid = int(grid)
...
def tune_object_program(args: argparse.Namespace) -> None:
    ...
    tokenizer = load_tokenizer_for_dir(args.checkpoint)
...
def tune_policy(args: argparse.Namespace) -> None:
    ...
    out = Path(args.out_dir)
    out.mkdir(parents=True, exist_ok=True)
    save_model(model, out, {"image_policy_tune_steps": float(args.steps), **last_metrics}, name="model")
    print(json.dumps({"done": True, "out_dir": str(out)}, indent=2), flush=True)
...
def _add_reward_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--augment_preferences", action="store_true", help="apply conservative low-data preference expansion")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="NEED image preference reward training and low-data policy tuning")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if args.cmd in ("train_reward", "reward", "train"):
        train_reward(args)

...
from need_core import (
    NeedConfig, NeedModel, ByteTokenizer, Special, make_image_tokenizer,
...
class TextChunkDataset(Dataset):
    def __init__(self, tokens: Sequence[int], block_size: int, predict_heads: int = 1, samples: int = 10000, seed: int = 123):
        ...
        self.seed = int(seed)
        if len(self.tokens) < self.block_size + self.predict_heads + 1:
            reps = int(math.ceil((self.block_size + self.predict_heads + 1) / max(1, len(self.tokens))))
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        max_start = len(self.tokens) - self.block_size - self.predict_heads - 1
...
class StreamingTextChunkDataset(IterableDataset):
    ...
    loading the corpus into host RAM, which is required for 85B-token runs.
    """
    def __init__(
        self,
        path: Path,
    ...
    def __iter__(self):
        ...
        buf: List[int] = []
        bytes_seen = 0
        need = self.block_size + self.predict_heads + 1
        while True:
            made_progress = False
            for file_index, path in enumerate(files):
                if file_index % nw != wid:
                    continue
                try:
...
def _numpy_dtype_for_tokens(name: str, vocab_size: int = 0) -> np.dtype:
    ...
    if name == "auto":
        return np.dtype(np.uint16 if int(vocab_size or 0) <= 65535 else np.uint32)
    if name in ("u16", "uint16"):
        return np.dtype(np.uint16)
    if name in ("i32", "int32"):
        return np.dtype(np.int32)
    if name in ("u32", "uint32"):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for path in files:
                ...
                if usable <= 0:
                    continue
                start = min(usable, wid * self.block_size)
                while start <= usable:
...
class PackedTokenMapDataset(Dataset):
    def __init__(self, path: Path, cfg: NeedConfig, predict_heads: int = 1, samples: int = 1024, seed: int = 123, dtype_name: str = "auto"):
        ...
        self.usable = [max(0, int(a.shape[0]) - (self.block_size + self.predict_heads + 1)) for a in self.arrays]
...
def apply_profile(args: argparse.Namespace) -> None:
    profiles = {
        "tiny": dict(d_model=16, n_layers=1, n_heads=4, block_size=8, d_ff=32, n_experts=1, energy_rank=4, memory_slots=2, pathway_memory_slots=4, planner_horizons=1),
        "small": dict(d_model=192, n_layers=4, n_heads=6, block_size=256, d_ff=768, n_experts=4, energy_rank=64, memory_slots=16, pathway_memory_slots=16),
        "medium": dict(d_model=384, n_layers=8, n_heads=6, block_size=512, d_ff=1536, n_experts=4, energy_rank=96, memory_slots=32, pathway_memory_slots=24),
        "large": dict(d_model=768, n_layers=16, n_heads=12, block_size=1024, d_ff=3072, n_experts=8, energy_rank=192, memory_slots=64, pathway_memory_slots=48),
        "reasoning": dict(d_model=512, n_layers=12, n_heads=8, block_size=1024, d_ff=2048, n_experts=8, energy_rank=160, memory_slots=64, planner_horizons=8, latent_slots=6, energy_routes=8, compute_budget=0.88, latent_search_depth=2),
        "image": dict(d_model=512, n_layers=10, n_heads=8, block_size=1536, d_ff=2048, n_experts=6, energy_rank=128, memory_slots=48, image_grid=24, image_max_grid=32, image_max_tokens=1024, compute_budget=0.82),
        "speed": dict(d_model=256, n_layers=6, n_heads=8, block_size=512, d_ff=1024, n_experts=2, energy_rank=64, memory_slots=16, energy_steps=2, compute_budget=0.55),
        "long_context": dict(d_model=384, n_layers=10, n_heads=6, block_size=2048, d_ff=1536, n_experts=4, energy_rank=96, memory_slots=64, memory_chunk_size=64, pathway_memory_slots=64),
        "multimodal": dict(d_model=512, n_layers=12, n_heads=8, block_size=1536, d_ff=2048, n_experts=8, energy_rank=160, memory_slots=64, image_grid=24, image_max_tokens=1024, energy_routes=8, object_program_slots=12),
        "agentic": dict(d_model=512, n_layers=12, n_heads=8, block_size=1024, d_ff=2048, n_experts=8, energy_rank=160, memory_slots=64, planner_horizons=8, latent_slots=8, energy_routes=10, aux_score_candidate_pool=16, compute_budget=0.90),
        "need_0_6b_dense": dict(d_model=960, n_layers=19, n_heads=15, block_size=1024, d_ff=3840, n_experts=1, moe_top_k=1, disable_shared_expert=True, energy_rank=240, memory_rank=160, memory_slots=64, memory_chunk_size=64, pathway_memory_slots=64, energy_steps=2, energy_min_steps=1, compute_budget=0.70, dropout=0.05, disable_exact_recall=True, disable_image=True, image_ratio=0.0, lambda_moe_balance=0.0, lambda_branch_entropy=0.0, lambda_moe_router_z=0.0, lr=1.6e-4, weight_decay=0.10, amp="bf16", compile=True, compile_mode="max-autotune", compile_cudagraphs=True, compile_dynamic=False, auto_optimize=True, auto_batch=True, batch_size=0, grad_accum_steps=0, target_effective_batch_tokens=1_048_576, stream_data=True, num_workers=-1, prefetch_to_device=True, drop_last=True, minimal_aux_metrics=True, peak_tflops=209.0),
        "need_0_6b_dense_85b": dict(d_model=960, n_layers=19, n_heads=15, block_size=1024, d_ff=3840, n_experts=1, moe_top_k=1, disable_shared_expert=True, energy_rank=240, memory_rank=160, memory_slots=64, memory_chunk_size=64, pathway_memory_slots=64, energy_steps=2, energy_min_steps=1, compute_budget=0.70, dropout=0.05, disable_exact_recall=True, disable_image=True, image_ratio=0.0, lambda_moe_balance=0.0, lambda_branch_entropy=0.0, lambda_moe_router_z=0.0, lr=1.6e-4, weight_decay=0.10, amp="bf16", compile=True, compile_mode="max-autotune", compile_cudagraphs=True, compile_dynamic=False, auto_optimize=True, auto_batch=True, batch_size=0, grad_accum_steps=0, target_effective_batch_tokens=1_048_576, target_tokens=85_000_000_000, stream_data=True, num_workers=-1, prefetch_to_device=True, drop_last=True, minimal_aux_metrics=True, peak_tflops=209.0),
    }
    if args.profile and args.profile != "custom":
...
def build_config(args: argparse.Namespace) -> NeedConfig:
    apply_profile(args)
    ...
    cfg = NeedConfig(
        ...
        vocab_size=Special.text_vocab + args.image_codebook_size,
        ...
        memory_decay_short_min=args.memory_decay_short_min,
        memory_decay_short_max=args.memory_decay_short_max,
        memory_decay_episodic_min=args.memory_decay_episodic_min,
        memory_decay_episodic_max=args.memory_decay_episodic_max,
        memory_boundary_strength=args.memory_boundary_strength,
        ...
        exact_recall_token_bias=args.exact_recall_token_bias,
        exact_recall_bigram_bias=args.exact_recall_bigram_bias,
        exact_recall_chunk_size=args.exact_recall_chunk_size,
        ...
        objective_gradient_guard=not args.disable_objective_gradient_guard,
    ...
    # Lambda overrides
    for name in list(NeedConfig.__dataclass_fields__.keys()):
        if name.startswith("lambda_") and hasattr(args, name):
            setattr(cfg, name, getattr(args, name))
    # Ablations
    if args.disable_energy:
        cfg.lambda_equilibrium = 0.0; cfg.lambda_energy = 0.0; cfg.energy_steps = 1
    if args.disable_diffusion:
        cfg.lambda_diffusion = 0.0; cfg.lambda_image_diffusion = 0.0
    if args.disable_geodesic:
        cfg.lambda_geodesic = 0.0; cfg.lambda_path_straightness = 0.0; cfg.lambda_path_contractive = 0.0
    if args.disable_moe:
        cfg.n_experts = 1; cfg.moe_top_k = 1; cfg.moe_use_shared_expert = False; cfg.lambda_moe_balance = 0.0
    if args.disable_memory:
        cfg.memory_mix = 0.0; cfg.lambda_memory_entropy = 0.0; cfg.lambda_memory_diversity = 0.0
    if args.disable_planner:
        cfg.planner_horizons = 0; cfg.lambda_latent_planning = 0.0; cfg.lambda_planner_ce = 0.0
    if args.disable_aux_score:
        cfg.lambda_aux_score = 0.0; cfg.aux_score_logit_scale = 0.0
    if args.disable_mtp:
        cfg.n_predict_heads = 1; cfg.lambda_mtp = 0.0
    if args.disable_adaptive_compute:
        cfg.adaptive_energy = False; cfg.adaptive_depth = False; cfg.lambda_adaptive_effort = 0.0; cfg.lambda_compute_budget = 0.0
    if args.disable_energy_routes:
        cfg.energy_routes = 1; cfg.energy_route_strength = 0.0; cfg.lambda_mixture_energy_router = 0.0
    if args.disable_latent_slots:
        cfg.latent_slots = 1; cfg.latent_slot_conditioning_scale = 0.0; cfg.lambda_latent_slot = 0.0; cfg.lambda_latent_slot_diversity = 0.0
    if args.disable_risk_signal_fusion:
        cfg.lambda_risk_signal = 0.0; cfg.lambda_latent_divergence = 0.0; cfg.risk_gate_strength = 0.0
    if args.disable_image:
        cfg.lambda_image_diffusion = 0.0; cfg.lambda_image_contrastive = 0.0; cfg.lambda_image_local_contrastive = 0.0; cfg.lambda_region_word_alignment = 0.0; cfg.lambda_image_spatial_smoothness = 0.0; cfg.lambda_object_program = 0.0; cfg.image_2d_scan = False; cfg.image_coord_scale = 0.0
    cfg.validate()
    return cfg
...
def verify_packed_index_integrity(index_path: Path, *, strict: bool = True) -> Dict[str, Any]:
    ...
    dataset drift before an 85B-token run.
    """
...
def _build_text_eval_loader(name: str, path: Path, cfg: NeedConfig, tokenizer: ByteTokenizer, args: argparse.Namespace, device: torch.device) -> DataLoader:
    ...
    if path.suffix == ".json" and _load_json_maybe(path).get("format", "").startswith("need_source_balanced"):
        ds = SourceBalancedPackedMapDataset(path, cfg, cfg.n_predict_heads, samples=samples, seed=args.seed + abs(hash(name)) % 100000, dtype_name=args.packed_dtype)
    elif path.suffix in (".bin", ".tokens", ".tokbin"):
        ds = PackedTokenMapDataset(path, cfg, cfg.n_predict_heads, samples=samples, seed=args.seed + abs(hash(name)) % 100000, dtype_name=args.packed_dtype)
    else:
        ids = read_text_tokens(path, tokenizer, max_bytes=int(getattr(args, "eval_data_bytes", 0) or 64 * 1024 * 1024))
        ds = TextChunkDataset(ids, cfg.block_size, cfg.n_predict_heads, samples=samples, seed=args.seed + abs(hash(name)) % 100000)
...
OBJECTIVE_FAMILY_LAMBDAS: Dict[str, Tuple[str, ...]] = {
    "prediction": (
        "lambda_mtp", "lambda_dvsd_slot_ce", "lambda_dvsd_consistency", "lambda_dvsd_router",
        "lambda_latent_planning", "lambda_planner_ce", "lambda_planning_consistency", "lambda_diffusion",
    ),
    "latent": (
        "lambda_latent_slot", "lambda_latent_slot_diversity", "lambda_mixture_energy_router", "lambda_timescale",
    ),
    "risk": (
        "lambda_risk_signal", "lambda_latent_divergence", "lambda_output_mode_classifier",
        "lambda_aux_score", "lambda_controller",
    ),
    "vision": (
        "lambda_image_diffusion", "lambda_image_contrastive", "lambda_image_local_contrastive",
        "lambda_region_word_alignment", "lambda_image_spatial_smoothness", "lambda_object_program",
    ),
    "regularizer": (
        "lambda_equilibrium", "lambda_energy", "lambda_moe_balance", "lambda_moe_router_z",
        "lambda_branch_entropy", "lambda_conv_scale_entropy", "lambda_geodesic",
        "lambda_path_straightness", "lambda_path_contractive", "lambda_energy_row_orth",
        "lambda_latent_norm", "lambda_memory_entropy", "lambda_memory_diversity",
        "lambda_adaptive_effort", "lambda_compute_budget", "lambda_pathway_memory",
        "lambda_state_drift", "lambda_state_anchor", "lambda_recall_entropy",
    ),
}
...
def train(args: argparse.Namespace) -> None:
    # Apply presets before anything else so auto-sized/model-specific defaults are active.
    apply_profile(args)
    if getattr(args, "recipe", "") and apply_training_recipe is not None:
        apply_training_recipe(args, args.recipe)
    maybe_apply_target_model_size(args)
    ...
    if getattr(args, "resume_from", ""):
        ...
        current_cfg = asdict(build_config(args))
    ...
    tokenizer = ByteTokenizer()
    save_json({"type": "byte", "special": {"pad": Special.pad, "bos": Special.bos, "eos": Special.eos}}, out / "tokenizer.json")
...
def self_test(device_name: str = "cpu") -> None:
    ...
    cfg = NeedConfig(
        ...
        lambda_diffusion=0.0, lambda_image_contrastive=0.0,
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--profile", choices=["custom", "tiny", "small", "medium", "large", "reasoning", "image", "speed", "long_context", "multimodal", "agentic", "need_0_6b_dense", "need_0_6b_dense_85b"], default="custom")
    ...
    p.add_argument("--target_params", type=str, default="", help="Auto-size model parts for a target count such as 600M or 0.6B.")
    ...
    p.add_argument("--memory_decay_short_min", type=float, default=0.50)
    p.add_argument("--memory_decay_short_max", type=float, default=0.97)
    p.add_argument("--memory_decay_episodic_min", type=float, default=0.78)
    p.add_argument("--memory_decay_episodic_max", type=float, default=0.997)
    p.add_argument("--memory_boundary_strength", type=float, default=0.70)
    p.add_argument("--pathway_conditioning_top_k", type=int, default=64)
    ...
    p.add_argument("--pathway_conditioning_max_vectors", type=int, default=512)
    ...
    p.add_argument("--region_word_sinkhorn_iters", type=int, default=4)
    ...
    p.add_argument("--disable_parallel_scan", action="store_true", help="Use reference Python recurrent loops instead of associative-scan parallel recurrences")
    ...
    p.add_argument("--exact_recall_dim", type=int, default=64)
    p.add_argument("--exact_recall_top_k", type=int, default=8)
    p.add_argument("--exact_recall_mix", type=float, default=0.10)
    p.add_argument("--exact_recall_temperature", type=float, default=0.10)
    p.add_argument("--exact_recall_token_bias", type=float, default=1.75)
    p.add_argument("--exact_recall_bigram_bias", type=float, default=0.75)
    p.add_argument("--exact_recall_chunk_size", type=int, default=128)
    p.add_argument("--exact_recall_max_tokens", type=int, default=4096)
    ...
    p.add_argument("--disable_objective_gradient_guard", action="store_true", help="Disable occasional CE-vs-aux gradient conflict probes")
    ...
    # objective weights
    defaults = NeedConfig()
    for name in NeedConfig.__dataclass_fields__:

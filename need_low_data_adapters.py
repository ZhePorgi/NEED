...
latent replay memory weighting.
"""
...
from need_core import ByteTokenizer, NeedModel, LatentMemoryStore, load_model, resolve_device, save_json, save_model
...
def make_runtime_profile(args: argparse.Namespace) -> None:
    ...
    if args.interactions:
        ...
        profile["behavioral_replay_min_score_hint"] = max(0.35, min(0.70, _quantile(scores, 0.35, 0.45)))
    ...
    if args.replay_dataset:
        profile["sources"]["replay_dataset"] = {
            "path": args.replay_dataset,
            "k_hint": int(args.replay_context_k),
            "similarity_hint": str(args.replay_context_similarity),
            "note": "recorded as an available behavioral-replay source; runtime replay remains opt-in",
        }
    out = Path(args.out_file)
    out.parent.mkdir(parents=True, exist_ok=True)
    save_json(profile, out)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rp.add_argument("--replay_dataset", default="")
    rp.add_argument("--replay_context_k", type=int, default=3)
    rp.add_argument("--replay_context_similarity", choices=["latent", "text", "hybrid"], default="hybrid")

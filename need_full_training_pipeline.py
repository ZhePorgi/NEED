...
def curriculum_cmd(args: argparse.Namespace, corpus_dir: Path, train_dir: Path, image_tokens: str) -> List[str]:
    ...
    _append(cmd, "--profile", args.need_profile)
...
def eval_suite_cmd(args: argparse.Namespace, final_ckpt: str, corpus_dir: Path, raw_dir: Path, post_dir: Path, eval_dir: Path, audit_dir: Path) -> List[str]:
    image_tokens = str(raw_dir / "tokens" / "image_tokens.val.jsonl") if (raw_dir / "tokens" / "image_tokens.val.jsonl").exists() else str(raw_dir / "tokens")
    sidecar_path = post_dir / "adapters" / "sidecar_need_latent"
    if not sidecar_path.exists():
        sidecar_path = post_dir / "sidecar_need_latent"
    cmd = [
        sys.executable, str(SCRIPT_DIR / "need_eval_suite.py"),
        "--checkpoint", final_ckpt,
        "--out_dir", str(eval_dir),
        "--audit_json", str(audit_dir / "audit_report.json"),
        "--json_artifacts", str(corpus_dir),
        "--image_tokens", image_tokens,
        "--sidecar_latent_alignment_path", str(sidecar_path),
        "--device", args.device,
        "--kernel_backend", args.kernel_backend,
    ]
    if args.prefer_best_eval:
        cmd.append("--prefer_best")
    if args.skip_model_eval or args.dry_run:
        cmd.append("--skip_model")
    if args.baseline_scorecard:
        _append(cmd, "--baseline_scorecard", args.baseline_scorecard)
    if args.strict_eval:
        cmd.append("--strict")
    return cmd
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.action == "eval":
        final_ckpt = args.need_checkpoint or expected_final_checkpoint(train_dir, include_rl_td_end=bool(args.include_rl_td_end))
        _run_stage(name="eval_suite", cmd=eval_suite_cmd(args, final_ckpt, corpus_dir, raw_dir, post_dir, eval_dir, audit_dir), log_path=logs / "91_eval_suite.log", env=env, cwd=SCRIPT_DIR, state_path=state_path, state=state, records=records, resume=resume, dry_run=False)
        return {"out_root": str(out_root), "records": records}
    if args.action == "card":
    ...
    if not args.skip_eval_suite:
        _run_stage(name="eval_suite", cmd=eval_suite_cmd(args, final_ckpt, corpus_dir, raw_dir, post_dir, eval_dir, audit_dir), log_path=logs / "05_eval_suite.log", env=env, cwd=SCRIPT_DIR, state_path=state_path, state=state, records=records, resume=resume, dry_run=args.dry_run and args.dry_plan_only)
    else:
        _run_stage(name="eval_suite", cmd=None, log_path=logs / "05_eval_suite.log", env=env, cwd=SCRIPT_DIR, state_path=state_path, state=state, records=records, resume=resume, dry_run=False, skipped_reason="--skip_eval_suite")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("action", nargs="?", choices=["start", "resume", "status", "audit", "eval", "card"], default="start")
    ...
    p.add_argument("--need_profile", choices=["custom", "tiny", "small", "medium", "large", "reasoning", "image", "speed", "long_context", "multimodal", "agentic"], default="multimodal")
    ...
    p.add_argument("--skip_eval_suite", action="store_true")
    p.add_argument("--strict_eval", action="store_true", help="Fail the pipeline on eval regression/failure.")
    p.add_argument("--prefer_best_eval", action="store_true")
    p.add_argument("--skip_model_eval", action="store_true", help="Run artifact/runtime eval only; do not load the checkpoint.")
    p.add_argument("--baseline_scorecard", default="", help="Previous evals/scorecard.json for regression detection.")

...
class VisualTokenizerConfig:
    ...
    def validate(self) -> None:
        if self.downsample & (self.downsample - 1) != 0:
        ...
        if self.max_grid < self.min_grid:
            raise ValueError("max_grid must be >= min_grid")
...
class ResBlock(nn.Module):
    def __init__(self, ch: int):
        ...
        self.net = nn.Sequential(
            nn.GroupNorm(max(1, min(32, ch // 4)), ch), nn.SiLU(), nn.Conv2d(ch, ch, 3, padding=1),
            nn.GroupNorm(max(1, min(32, ch // 4)), ch), nn.SiLU(), nn.Conv2d(ch, ch, 3, padding=1),
        )
...
class VisualTokenizerVQVAE(nn.Module):
    def __init__(self, cfg: VisualTokenizerConfig):
        ...
        dec += [nn.GroupNorm(max(1, min(32, h // 4)), h), nn.SiLU(), nn.Conv2d(h, 3, 3, padding=1)]
...
def pil_to_tensor(img: "Image.Image", size: int) -> torch.Tensor:
    ...
    img = img.convert("RGB").resize((size, size), resample=Image.Resampling.LANCZOS)
...
class PatchDiscriminator(nn.Module):
    def __init__(self, base: int = 64):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(base * 2, base * 4, 4, stride=2, padding=1), nn.GroupNorm(16, base * 4), nn.LeakyReLU(0.2, inplace=True),
...
def load_visual_tokenizer(path: Union[str, Path], device: Union[str, torch.device] = "cpu") -> VisualTokenizerVQVAE:
    ...
    if safe_load_file is not None and (root / "visual_tokenizer.safetensors").exists():
        state = safe_load_file(str(root / "visual_tokenizer.safetensors"), device=str(device))
    elif (root / "visual_tokenizer.pt").exists():
        try:
            state = torch.load(root / "visual_tokenizer.pt", map_location=device, weights_only=True)
        except TypeError:
            state = torch.load(root / "visual_tokenizer.pt", map_location=device)
    else:

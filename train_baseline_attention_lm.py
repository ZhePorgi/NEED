...
class AttentionBaselineLM(nn.Module):
    ...
    def forward(self,x,y=None):
        ...
        if y is not None:
            per=F.cross_entropy(logits.reshape(-1,self.cfg.vocab_size), y.reshape(-1), ignore_index=Special.pad, reduction='none').view_as(y)
            valid=(y!=Special.pad).float(); loss=(per*valid).sum()/valid.sum().clamp_min(1.0)
        return logits, loss

#!/usr/bin/env python3
"""Generate text or learned-token images with the model."""
...
def _apply_runtime_profile(args: argparse.Namespace) -> dict:
    ...
    safe_keys = {
        ...
        "decode_mode", "streaming_cache", "nonseq_decode", "nonseq_dynamic", "nonseq_min_heads", "nonseq_max_heads",
        ...
        "dvsd_slot_order", "dvsd_freeze_sampled_slots",
...
def _apply_dvsd_runtime_overrides(model, args) -> dict:
    ...
    mapping = {
        ...
        "dvsd_planner_compound_enabled": getattr(args, "dvsd_planner_compound_enabled", None),
        "dvsd_planner_compound_mix": getattr(args, "dvsd_planner_compound_mix", None),
        "dvsd_planner_compound_step_size": getattr(args, "dvsd_planner_compound_step_size", None),
        "dvsd_planner_compound_token_scale": getattr(args, "dvsd_planner_compound_token_scale", None),
        "dvsd_planner_compound_descent_scale": getattr(args, "dvsd_planner_compound_descent_scale", None),
        "dvsd_planner_compound_top_k": getattr(args, "dvsd_planner_compound_top_k", None),
        "planner_block_space_enabled": getattr(args, "planner_block_space_enabled", None),
        "planner_block_space_mix": getattr(args, "planner_block_space_mix", None),
        "planner_block_space_iters": getattr(args, "planner_block_space_iters", None),
        "dvsd_slot_order": getattr(args, "dvsd_slot_order", None),
        "dvsd_freeze_sampled_slots": getattr(args, "dvsd_freeze_sampled_slots", None),
    }
    bool_keys = {"dvsd_router_enabled", "dvsd_planner_compound_enabled", "planner_block_space_enabled", "dvsd_freeze_sampled_slots"}
    int_keys = {"dvsd_planner_compound_top_k", "planner_block_space_iters"}
    str_keys = {"dvsd_slot_order"}
    ...
    if hasattr(model, "_dvsd_planner_compound_loaded"):
        applied.setdefault("dvsd_planner_compound_loaded", bool(getattr(model, "_dvsd_planner_compound_loaded")))
    if hasattr(model, "_planner_block_space_loaded"):
        applied.setdefault("planner_block_space_loaded", bool(getattr(model, "_planner_block_space_loaded")))
    try:
        model.cfg.validate()
    except Exception:
        pass
...
def _sidecar_should_run(args, metrics: dict) -> tuple[bool, dict]:
    policy = str(getattr(args, "sidecar_call_policy", "off") or "off").lower()
...
def _controller_runtime_adjust(model, tok: ByteTokenizer, prompt: str, args, device: torch.device) -> dict:
    ...
    if action == "retrieve":
        ...
        args.latent_search_depth = int(args.latent_search_depth)
        args.reasoning_tree_branches = int(args.reasoning_tree_branches)
        args.aux_score_candidate_pool = int(args.aux_score_candidate_pool)
        args.aux_score_weight = float(args.aux_score_weight)
        applied["runtime_changes"] = {
            "behavioral_recall_only": True,
            "linear_core_preserved": True,
        ...
        args.latent_search_depth = int(args.latent_search_depth)
        args.latent_search_branches = int(args.latent_search_branches)
        args.reasoning_tree_branches = int(args.reasoning_tree_branches)
        args.aux_score_candidate_pool = int(args.aux_score_candidate_pool)
        applied["runtime_changes"] = {"linear_core_preserved": True, "latent_search_depth": args.latent_search_depth, "latent_search_branches": args.latent_search_branches, "reasoning_tree_branches": args.reasoning_tree_branches, "aux_score_candidate_pool": args.aux_score_candidate_pool}
    elif action == "revise":
        args.aux_score_weight = float(args.aux_score_weight)
        args.aux_score_backtrack_window = int(args.aux_score_backtrack_window)
        args.aux_score_max_backtracks = int(args.aux_score_max_backtracks)
...
def _make_dual_channel_context(model, tok: ByteTokenizer, prompt: str, args, device: torch.device, sidecar_rt=None):
    ...
    if sidecar_rt is not None and getattr(sidecar_rt, "source_type", "external_lm") == "need" and bool(getattr(args, "use_need_sidecar_latents", False)):
        try:
...
def _need_fallback_one_token(model, idx: torch.Tensor, args, conditioning_vectors, conditioning_scale: float, generated_so_far: int) -> torch.Tensor:
    ...
    with need_optimization_mode():
        out = model.generate_text(
            ...
            use_streaming_cache=bool(getattr(args, "streaming_cache", True)),
        )
...
def run_text(args: argparse.Namespace) -> None:
    ...
    if args.speculative_final_decode and sidecar_rt is not None and bool(getattr(sidecar_rt, "supports_speculative_final_decode", True)):
        ...
        mode = str(getattr(args, "decode_mode", "ar") or "ar")
        ...
        with need_optimization_mode():
            if use_nonseq:
                out, nonseq_metrics = model.generate_text_nonsequential(
                    ...
                    dvsd_planner_compound_enabled=args.dvsd_planner_compound_enabled,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--controller_runtime_policy", choices=["off", "observe", "auto"], default="off", help="Use the controller head to observe or automatically adjust retrieve/deepen/revise generation knobs")
    ...
    p.add_argument("--dual_channel_reasoning", action=argparse.BooleanOptionalAction, default=False)
    p.add_argument("--hide_thought_summary", action="store_true")
    p.add_argument("--vector_stride", type=int, default=2)
    p.add_argument("--max_vectors", type=int, default=64)
    ...
    p.add_argument("--decode_mode", choices=["auto", "ar", "nonseq"], default="ar", help="Default ar uses the stateful streaming cache when supported; nonseq/DVSD is opt-in, and auto only selects nonseq outside strict streaming core")
    p.add_argument("--streaming_cache", action=argparse.BooleanOptionalAction, default=True, help="Use the stateful NEED text cache for AR decoding when supported; --no-streaming_cache forces the compatibility full-context path")
    ...
    p.add_argument("--aux_score_weight", type=float, default=0.0)
    ...
    p.add_argument("--nonseq_tree_candidates", type=int, default=1, help="Legacy aux_scored-draft knob retained for compatibility; ignored by the default virtual-slot decoder")
    p.add_argument("--nonseq_branch_top_k", type=int, default=1)
    p.add_argument("--nonseq_aux_score_weight", type=float, default=0.0)
    ...
    p.add_argument("--dvsd_planner_compound_enabled", action=argparse.BooleanOptionalAction, default=None, help="Feed provisional DVSD tokens through the latent planner so later slots benefit from compounded token-descent updates")
    p.add_argument("--dvsd_planner_compound_mix", type=float, default=None, help="Blend compounded-planner logits into future DVSD slot logits; default keeps checkpoint/config setting")
    p.add_argument("--dvsd_planner_compound_step_size", type=float, default=None, help="Latent planner transition step size for DVSD token compounding")
    p.add_argument("--dvsd_planner_compound_token_scale", type=float, default=None, help="Strength of token-feedback residuals in DVSD planner compounding")
    p.add_argument("--dvsd_planner_compound_descent_scale", type=float, default=None, help="Strength of approximate logit-descent residuals in DVSD planner compounding")
    p.add_argument("--dvsd_planner_compound_top_k", type=int, default=None, help="Top-k logits used to approximate the output-embedding expectation for DVSD descent")
    p.add_argument("--planner_block_space_enabled", action=argparse.BooleanOptionalAction, default=None, help="Use full-block latent planning where all future slots interact before logits are produced")
    p.add_argument("--planner_block_space_mix", type=float, default=None, help="Blend weight for block-space planner states vs recurrent horizon planner states")
    p.add_argument("--planner_block_space_iters", type=int, default=None, help="Number of cheap slot-attention iterations inside the full-block latent planner")
    p.add_argument("--dvsd_slot_order", choices=["confidence", "left_to_right"], default=None, help="Order for filling the private DVSD canvas; confidence allows non-monotonic anchor-first slot filling")
    p.add_argument("--dvsd_freeze_sampled_slots", action=argparse.BooleanOptionalAction, default=None, help="Keep sampled DVSD slots fixed inside the block before direct commit")
    # Single active sidecar. Exactly one backend is loaded: none, external_lm, or a smaller NEED checkpoint.
    p.add_argument("--sidecar_type", choices=["auto", "none", "external_lm", "need"], default="none", help="Select the one active sidecar backend. auto prefers --need_sidecar_checkpoint, otherwise --sidecar_model")
    ...
    p.add_argument("--need_sidecar_decode_mode", choices=["nonseq", "ar"], default="ar", help="Decoder used by the smaller NEED sidecar when it drafts public summaries")
    ...
    p.add_argument("--use_need_sidecar_latents", action=argparse.BooleanOptionalAction, default=False, help="Append projected latent anchors from a smaller NEED sidecar when active")
    p.add_argument("--sidecar_call_policy", choices=["always", "latent_gated", "off"], default="off", help="When to call the single active sidecar; latent_gated skips it on easy prompts")
    ...
    p.add_argument("--sidecar_cot", action=argparse.BooleanOptionalAction, default=False)
    ...
    p.add_argument("--store_raw_cot_history", action=argparse.BooleanOptionalAction, default=False)
    ...
    p.add_argument("--speculative_final_decode", action=argparse.BooleanOptionalAction, default=False, help="Use sidecar-drafted final answer blocks validated by NEED; active only when an external LM sidecar is loaded")
    ...
    p.add_argument("--adaptive_spec_acceptance", action=argparse.BooleanOptionalAction, default=False, help="Derive speculative acceptance strictness from latent convergence and live acceptance feedback")

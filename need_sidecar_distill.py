...
from need_core import ByteTokenizer, NeedConfig, NeedModel, load_model, save_model, save_json, resolve_device
...
class SidecarDistillDataset(Dataset):
    def __init__(self, texts: Sequence[str], tokenizer: ByteTokenizer, block_size: int, predict_heads: int, samples: int, seed: int = 123) -> None:
        ...
        need = self.block_size + self.predict_heads + 2
        if len(ids) < need:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        max_start = max(0, int(self.ids.numel()) - self.block_size - self.predict_heads - 1)
...
def make_sidecar_config(args: argparse.Namespace, teacher_cfg: NeedConfig) -> NeedConfig:
    if args.student_config:
        ...
        cfg = NeedConfig(**{k: v for k, v in raw.items() if k in NeedConfig.__dataclass_fields__})
        cfg.validate()
        return cfg
    if args.target_params and model_config_for_params is not None:
        ...
        cfg = NeedConfig(**{k: v for k, v in base.items() if k in NeedConfig.__dataclass_fields__})
        cfg.validate()
        return cfg
...
def kl_divergence_student_teacher(student_logits: torch.Tensor, teacher_logits: torch.Tensor, temperature: float, vocab_limit: int) -> torch.Tensor:
    s = student_logits[..., :vocab_limit].float() / float(temperature)
    t = teacher_logits[..., :vocab_limit].float() / float(temperature)
...
def train(args: argparse.Namespace) -> None:
    ...
    while step < args.steps:
        for batch in dl:
            ...
            ce = s_loss if s_loss is not None else F.cross_entropy(s_logits.reshape(-1, s_logits.size(-1)), y[..., 0].reshape(-1), ignore_index=student.cfg.pad_id)
            loss = loss + float(args.ce_weight) * ce
            logit_kl = kl_divergence_student_teacher(s_logits, t_logits[:, -s_logits.size(1):], args.temperature, vocab_limit)
            ...
            if s_h is not None and t_h is not None and float(args.mtp_kl_weight) > 0:
                ...
                for i in range(1, n_heads):
                    parts.append(kl_divergence_student_teacher(s_mtp[i], t_mtp[i][:, -s_mtp[i].size(1):], args.temperature, vocab_limit))
                if parts:

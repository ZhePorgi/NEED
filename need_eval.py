...
from need_core import ByteTokenizer, load_model, make_image_tokenizer, resolve_device, Special, load_tokenizer_for_dir
...
def eval_text(model, data_path: Path, device, batches: int, batch_size: int, tokenizer=None):
...
def latency(model, device, prompt: str, new_tokens: int, tokenizer=None):
    tok = tokenizer if tokenizer is not None else ByteTokenizer()
...
def main(argv: Optional[Sequence[str]]=None):
    ...
    results={}
    if args.data: results.update(eval_text(model,Path(args.data),device,args.batches,args.batch_size,tokenizer=tokenizer))
    if args.image_dir:
    ...
    results.update(latency(model,device,'The answer is',args.latency_tokens,tokenizer=tokenizer))
    if args.dashboard:

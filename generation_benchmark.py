...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    p.add_argument("--decode_mode", choices=["ar", "nonseq"], default="nonseq")
    ...
    for prompt in prompts:
        ...
        for _ in range(max(1, args.iters)):
            if args.decode_mode == "nonseq":
                ...
                nonseq_compound_steps += float(stats.get("nonseq_compound_steps", 0.0))
                nonseq_compound_blend += float(stats.get("nonseq_compound_blend_rate", 0.0))
            else:
        ...
        if args.decode_mode == "nonseq":
            ...
            row["dvsd_compound_steps_mean"] = nonseq_compound_steps / max(1.0, float(args.iters))
            row["dvsd_compound_blend_rate_mean"] = nonseq_compound_blend / max(1.0, float(args.iters))

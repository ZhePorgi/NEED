...
from need_core import ByteTokenizer, NeedConfig, build_default_tokenizer, DEFAULT_HF_TOKENIZER_MODEL, save_json
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    p.add_argument("--tokenizer", choices=["hf", "byte"], default="hf",
                    help="Tokenizer to pack tokens with; must match what train.py will use. "
                         f"'hf' (default) uses a Hugging Face tokenizer (default model: {DEFAULT_HF_TOKENIZER_MODEL}).")
    p.add_argument("--tokenizer_model", type=str, default="", help=f"HF hub id for --tokenizer=hf. Default: {DEFAULT_HF_TOKENIZER_MODEL}")
    p.add_argument("--vocab_size", type=int, default=0, help="Override vocab size; default derives from --tokenizer.")
    ...
    tok = build_default_tokenizer(args.tokenizer_model or None, kind=args.tokenizer)
    if not args.vocab_size:
        args.vocab_size = int(tok.vocab_size)
    save_json(tok.to_dict(), out_dir / "tokenizer.json")
    raw_weights = [max(0.0, float(s.get("weight", 1.0))) for s in sources]
    used_names: Dict[str, int] = {}
    wsum = sum(raw_weights)
    if wsum <= 0.0:
        norm_weights = [1.0 / float(len(raw_weights) or 1) for _ in raw_weights]
    else:

# NEED: Thought is a deep river

NEED stands for Nested Energy Equilibrium Descent. It's an AI model architecture I developed that aims to do a couple of things:
Firstly is that ever since the transformer has been used in frontier AIs, they've already mostly outgrown the original, simplistic structure of it. It was made for the simple task of translating text, but current models are expected to think ahead, reason, and remember in ways that the architecture could fundamentally improve with, so you could achieve similar performance without scaling increasingly larger. Secondly is that these improvements also let the model engagingly allocate compute, leading to more efficient generation. It's systematically been designed to try and engineer out flaws in the original transformer, with the most important one being that it reasons by following a path through latent space and not token space. This document explains essentially all of the NEED architecture.
